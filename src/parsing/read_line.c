...
t_bool  add_elem(t_scene *scene, char *str)
{
    if (str[0] && str[1])
    {
        if (str[0] == 'A')
...
t_bool  add_ambient(t_scene *scene, char *str)
{
    while (*str && *str == ' ')
        str++;
    if (*str)
        scene->ambient.lighting = ft_atof(&str);
    else
        return (true);
    if (errno)
        return (print_error(strerror(errno)));
    if (scene->ambient.lighting < 0 || scene->ambient.lighting > 1)
...
t_lights    *add_light(t_scene *scene, char *str)
{
    t_lights    *light;

    light = malloc(sizeof(t_lights));
    if (!light)
