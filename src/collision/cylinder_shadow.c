...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/03 10:36:04 by sabitbol          #+#    #+#             */
/*   Updated: 2025/01/06 14:14:26 by sabitbol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_point	intersection_cylinder_line_shadow(t_line *line, t_cylinder *cyl)
{
	...
	* va.y + va.z * va.z, 2 * (ra0.x * va.x + ra0.y * va.y + ra0.z * va.z), \
	ra0.x * ra0.x + ra0.y * ra0.y + ra0.z * ra0.z - cyl->radius * cyl->radius));
	disk = get_disk(line, cyl);
	if (disk.z != INFINITY || p.z == INFINITY)
		return (disk);
